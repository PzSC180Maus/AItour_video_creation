const COMMUNITY_API = {
  postList: "/data/community/post",
  cardList: "/data/community/card",
  cardUse: "/data/community/card/use",
  postPublish: "/data/community/post/publish",
  cardPublish: "/data/community/card/publish",
  profileMyPost: "/data/profile/mypost",
  profileMyCard: "/data/profile/mycard",
  profilePostLiked: "/data/profile/post_liked",
  profileCardLiked: "/data/profile/card_liked"
};

module.exports = COMMUNITY_API;

